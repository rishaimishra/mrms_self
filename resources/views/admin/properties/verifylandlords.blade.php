     ...
     <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <div class="card">
                    ...
                    <br>
                        <a  href="{{ route('admin.verify.landlord.rejected') }}" class="btn btn-large btn-danger pull-right" style="padding:10px; margin-left:10px;">Rejected</a>
                        <a  href="{{ route('admin.verify.landlord.approved') }}" class="btn btn-large btn-success pull-right" style="padding:10px; margin-left:10px;">Approved</a>
                        <a  href="{{ route('admin.verify.landlord') }}" class="btn btn-large btn-warning pull-right" style="padding:10px; margin-left:10px;">Pending</a>
                        <br>

                        
                        @if( $state == -2)
                            {!! Form::open(['method'=>'GET','url'=>route('admin.verify.landlord.rejected'),'class'=>'navbar-form navbar-left','role'=>'search'])  !!}
                        @endif
                        @if( $state == 1)
                            {!! Form::open(['method'=>'GET','url'=>route('admin.verify.landlord.approved'),'class'=>'navbar-form navbar-left','role'=>'search'])  !!}
                        @endif 
                        @if( $state == 0)
                            {!! Form::open(['method'=>'GET','url'=>route('admin.verify.landlord'),'class'=>'navbar-form navbar-left','role'=>'search'])  !!}
                        @endif
                    ...
                    <div class="row">
                        <div class="body table-responsive">
                                ...
                                <div class="card" style="padding:10px;">
                                    ...
                                    <h5>Property ID: {{ $user->property_id }}</h5>
                                    <div class="row">
                                            <div class="card-body col-xs-6 border">
                                                ...
                                                <p class="card-text"><b>Middle Name: </b> {{ $user->middle_name }}</p>
                                                <p class="card-text"><b>Surname: </b> {{ $user->surname }}</p>
                                                ...
                                                <p class="card-text"><b>Mobile_1: </b> {{ $user->mobile_1 }}</p>
                                            ...
                                            <div class="card-body col-xs-6 border">
                                                ...
                                                <p class="card-text"><b>First Name: </b> {{ $user->temp_first_name }}</p>
                                                <p class="card-text"><b>Middle Name: </b> {{ $user->temp_middle_name }}</p>
                                                <p class="card-text"><b>Surname: </b> {{ $user->temp_surname }}</p>
                                                ...
                                                <p class="card-text"><b>Mobile_1: </b> {{ $user->temp_mobile_1 }}</p>
                                                <p class="card-text"><b>Email: </b> {{ $user->temp_email }}</p>
                                            </div>
                                    </div>
                                            @if( $state == -2)
                                                <a href="{{ route('admin.verify.landlord.approve', $user->id) }}" class="btn btn-large btn-success">Approve</a>
                                            @endif
                                            @if( $state == 1)
                                                <a href="{{ route('admin.verify.landlord.reject', $user->id) }}" class="btn btn-large btn-danger">Reject</a>
                                            @endif 
                                            @if( $state == 0)
                                                <a href="{{ route('admin.verify.landlord.approve', $user->id) }}" class="btn btn-large btn-success">Approve</a>
                                                <a href="{{ route('admin.verify.landlord.reject', $user->id) }}" class="btn btn-large btn-danger">Reject</a>
                                    ...
                                    <div class="row">
                                            ...
                                            <h5>ID Proof</h5>
                                            <img src="{{ $user->getDocumentImagePathAttribute() }}" style="width:100%;cursor:zoom-in" onclick="loadImage('{{ $user->getDocumentImagePath() }}')">
                                            </div>
