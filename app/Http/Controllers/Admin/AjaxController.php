...
class AjaxController extends Controller
{
    ...
    public function calculateNewRate(Request $request)
    {
        ...
        if(isset($request->property_dimension) and $request->property_dimension != null)
        {
            ...
            $property_dimension = ($request->property_dimension);
        }
        ...
        $totalAddition = $wall_material['value'] + $roof_material['value'] + $value_added_val + $property_window_type_value['value']+ ($swimming_pool ? $swimming_pool : 0);
        ...
        $result['rateWithoutGST'] = (($propertyDimesion*$rate_square_meter)+$totalAddition) * $gated_community * $propertyUse * $property_sanitation_value['value'] * $step6 *$zone * $propertyType;
        ...
        $result['rateWithoutGST_Council'] = $result['rateWithoutGST'] * ((100-$property_council_adjustment)/100);
        
        $result['formatWithoutGST'] = number_format($result['rateWithoutGST'], 0, '', ',');
        $result['formatWithoutGST_Council'] = number_format($result['rateWithoutGST_Council'], 0, '', ',');
        $result['council_perc'] = $property_council_adjustment;
        $result['property_dimension'] = $property_dimension;
